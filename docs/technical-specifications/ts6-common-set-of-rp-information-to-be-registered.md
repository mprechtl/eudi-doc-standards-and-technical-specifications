...
## 2.2 Conditionally Mandatory Data to be Provided in Registration
...
| Use of Intermediary         | List of intermediaries the Wallet-Relying Party with entitlement type 'Service_provider' relies upon, if it is not transacting directly with the Wallet Units. When present, the intermediary-specific data object SHALL contain its unique identifier and its Trade Name.   | WalletRelyingParty.usesIntermediary  |
